...
const getData = (req, res, next) => {
    ...
    conn.query(sql, (_err, results) => {
        ...
        res.status(200).json({success: true, menus: results})
    })
}

const addData = (req, res, next) => {
    const sql = `insert into makanan(id, nama, harga, stok, tipe) values (?, ?, ?, ?, ?)`
    const id = uuid.v1()
    const {nama, harga, stok, tipe} = req.body
    if(tipe.toLowerCase() === 'makanan' || tipe.toLowerCase() === 'minuman') {
        conn.query(sql, [id, nama, parseInt(harga), parseInt(stok), tipe], (err, results) => {
            err ? 
            res.status(500).json({
                success: false,
                error: err
